...
class Base:
    ...
    def create_banner(self, game_name: str):
        ...
        {self.blue}Crypto Hunters {self.white}{game_name} Auto Claimer
        t.me/tgcryptohunters
        github.com/afridi315
    ...
    def set_username(self, data: str):
        start = data.find('user=') + len('user=')
        end = data.find('&', start)
        user_data = json.loads(urllib.parse.unquote(data[start:end]))
    ...
    def log(self, msg):
        ...
        print(f"{self.black}[{now}]{self.yellow}[{self.botname}]{self.green}{self.username}{self.reset} {msg}{self.reset}")
